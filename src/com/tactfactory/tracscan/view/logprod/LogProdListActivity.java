/**************************************************************************
 * LogProdListActivity.java, tracscan Android
 *
 * Copyright 2013
 * Description : 
 * Author(s)   : Harmony
...
/**
 * This class will display LogProd entities in a list.
 *
 * This activity has a different behaviour wether you're on a tablet or a phone.
 * - On a phone, you will have a LogProd list. A click on an item will get 
 * you to LogProdShowActivity.
 * - On a tablet, the LogProdShowFragment will be displayed right next to
 * the list.
 */
public class LogProdListActivity 
	...
	protected LogProdListFragment listFragment;
	/** Associated detail fragment if any (in case of tablet). */
	protected LogProdShowFragment detailFragment;
	/** Last selected item position in the list. */
	private int lastSelectedItemPosition = 0;
	/** Last selected item. */
	private LogProd lastSelectedItem;
	...
	protected void onPostCreate(Bundle savedInstanceState) {
		...
		this.detailFragment = (LogProdShowFragment) 
						this.getSupportFragmentManager().findFragmentById(
								R.id.fragment_show);

		this.listFragment = (LogProdListFragment)
						this.getSupportFragmentManager().findFragmentById(
								R.id.fragment_list);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		this.setContentView(R.layout.activity_logprod_list);
	}
	...
	public void onListItemClick(ListView l, View v, int position, long id) {
		...
		if (this.isDualMode()) {
			...
			final Intent intent = new Intent(this, LogProdShowActivity.class);
			final LogProd item = (LogProd) l.getItemAtPosition(position);
			Bundle extras = new Bundle();
			extras.putParcelable(LogProd.PARCEL, item);
	 ...
	 */
	private void loadDetailFragment(LogProd item) {
	...
	public void onListLoaded() {
		if (this.isDualMode()) {
			int newPosition =
				((LogProdListAdapter) this.listFragment.getListAdapter())
